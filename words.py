...
def detect_words_operations(image):
    ...
    treshold_image = cv2.adaptiveThreshold(
        image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 115, 40)
    blur_image = cv2.medianBlur(treshold_image, 5)
    input_image_to_morphological_transformations = blur_image
    erode_image = cv2.erode(
        input_image_to_morphological_transformations, kernel_ero, cv2.BORDER_REFLECT)
